...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    select_list = [obj for obj in paragraphs if select(obj)]
...
def about(subject):
    ...
    def helper(test_list):
        for word in subject:
            split_list = split(test_list)
            lower_list = [lower(words) for words in split_list]
            remove_punc_list = [remove_punctuation(i) for i in lower_list]
            if word in remove_punc_list:
                return True
        return False
    return helper
    # END PROBLEM 2
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:
        ...
        add = 1 + minimum_mewtations(typed, source[1:], limit - 1)  # 插入操作  
        remove = 1 + minimum_mewtations(typed[1:], source, limit - 1)  # 删除操作  
...
def report_progress(typed, source, user_id, upload):
    ...
    while count < typed_length and typed[count] == source[count]: 
        count += 1
    Progress = count / source_length
    grade_dic = {'id':user_id, 'progress':Progress}
    upload(grade_dic)
    return Progress
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = timestamps_per_player
    for j in range(len(timestamps_per_player)):
        time = [timestamps_per_player[j][i] - timestamps_per_player[j][i-1] for i in range(1,len(timestamps_per_player[j]))]
        times[j] = time
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        k = 0
        while time(match,k,i) != min(time_list):
            k += 1
        words[k] += [get_word(match,i)]
    return words
    # END PROBLEM 10
