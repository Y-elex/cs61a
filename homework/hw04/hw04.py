...
def balanced(m):
    ...
    def helper(x):
        left_rod = left(x)
        right_rod = right(x)
        left_length = length(left_rod)
        right_length = length(right_rod)
        left_mass = total_mass(end(left_rod))
        right_mass = total_mass(end(right_rod))
        if left_mass * left_length == right_mass * right_length:
            return True
        else:
            return False
    if helper(m):
...
def max_path_sum(t):
    ...
    if branches(t) == []:
        return label(t)
    s = [label(t) + max_path_sum(b) for b in branches(t)]
    return max(s)
