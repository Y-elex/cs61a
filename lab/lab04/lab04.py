def divide(quotients, divisors):
    ...
    """
    return {q: [x for x in divisors if x % q == 0] for q in quotients}
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    here = make_city('location', lat, lon)
    d1 = distance(here,city_a)
    d2 = distance(here,city_b)
    if d1 <= d2:
